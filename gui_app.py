...
import pandas as pd
from original_file import (analyze_excel_data, process_and_display_results,
                         analyze_excel_data_by_buyer, process_and_display_buyer_results)
            ...
            if self.total_radio.isChecked():
                ...
                if self.product_counts:
                    ...
                    for product, count in sorted(self.product_counts.items()):
                        ...
                        self.result_table.setItem(row_position, 0, QTableWidgetItem(product))
                        self.result_table.setItem(row_position, 1, QTableWidgetItem(f"{count}개"))
                ...
                # 구매자별 분석
                self.buyer_product_counts = analyze_excel_data_by_buyer(self.selected_file)
                if self.buyer_product_counts:
                    # 테이블 컬럼 설정
                    self.result_table.setColumnCount(2)
                    self.result_table.setHorizontalHeaderLabels(['구매자', '주문내역'])
                    
                    # 결과를 테이블에 표시
                    for buyer, product_counts in self.buyer_product_counts.items():
                        row_position = self.result_table.rowCount()
                        self.result_table.insertRow(row_position)
                        ...
                        self.result_table.setItem(row_position, 0, QTableWidgetItem(buyer))
                        self.result_table.setItem(row_position, 1, QTableWidgetItem(order_text))
        ...
        try:
            ...
            if self.total_radio.isChecked():
                ...
                if file_name and self.product_counts:
                    df = pd.DataFrame(list(self.product_counts.items()), columns=['상품명', '수량'])
                ...
                if file_name and self.buyer_product_counts:
                    ...
                    df = df.sort_values('구매자')
                    df.to_excel(file_name, index=False)
            
            if file_name:
                self.status_label.setText(f'엑셀 파일이 저장되었습니다: {os.path.basename(file_name)}')
